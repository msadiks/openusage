...
function makeSuccessResponse(credits, usableRequests) {
...
describe("crofai plugin", () => {
  ...
  it("returns credits and requests lines", async () => {
    const ctx = makeCtx()
    ctx.host.env.get.mockReturnValue("test-api-key")
    ctx.host.http.request.mockReturnValue(makeSuccessResponse(12.3456, 450))
    const plugin = await loadPlugin()
  ...
  it("handles usable_requests as string (casts to 0)", async () => {
    const ctx = makeCtx()
    ctx.host.env.get.mockReturnValue("test-api-key")
    ctx.host.http.request.mockReturnValue({
      status: 200,
  ...
  it("omits credits line when credits field is missing (defaults to 0)", async () => {
    const ctx = makeCtx()
    ctx.host.env.get.mockReturnValue("test-api-key")
    ctx.host.http.request.mockReturnValue({
      status: 200,
      bodyText: JSON.stringify({ usable_requests: 10 }),
    })
    const plugin = await loadPlugin()
    const result = plugin.probe(ctx)
